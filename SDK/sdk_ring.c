...
sdk_err_t sdk_ring_put(sdk_ring_t *ring, void* object)
{
    ...
    size_t next_write_idx = ring->write_idx + 1;
...
sdk_err_t sdk_ring_get(sdk_ring_t * ring, void* object){
    ...
    size_t next_read_idx = ring->read_idx+1;
...
sdk_err_t sdk_ring_read(sdk_ring_t * ring, size_t offset, void* object){
    size_t used = (ring->write_idx >= ring->read_idx) ?
                     (ring->write_idx - ring->read_idx) :
                     (ring->write_idx + ring->capacity - ring->read_idx);
    ...
    size_t read_idx = ring->read_idx + offset;
...
void* sdk_ring_peek(sdk_ring_t * ring, size_t offset /*0<= offset <used*/){
    ...
    size_t idx = ring->read_idx + offset;
